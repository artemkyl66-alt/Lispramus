             ...
             (let* ((t-offset (if (> t-total 1) (- (* (/ (node-height n2) t-total) t-idx) (/ (node-height n2) 2.5)) 0))
               ...
               (if (<= x1 x2)
                   ...
                   (let ((mid-y (+ (max y1 y2) 160 y-track)))
                     (incf y-track 35)
                     (push (make-edge :label label :side :input :x1 x1 :y1 y1 :x2 (+ x1 40) :y2 y1) all-edges)
                     (push (make-edge :label "" :side :input :x1 (+ x1 40) :y1 y1 :x2 (+ x1 40) :y2 mid-y) all-edges)
                     (push (make-edge :label "" :side :input :x1 (+ x1 40) :y1 mid-y :x2 (- x2 40) :y2 mid-y) all-edges)
                     (push (make-edge :label "" :side :input :x1 (- x2 40) :y1 mid-y :x2 (- x2 40) :y2 y2) all-edges)
                     (push (make-edge :label "" :side :input :rev t :x1 (- x2 40) :y1 y2 :x2 x2 :y2 y2) all-edges)))))
            (:control
               ...
               (if (eq n1 n2)
                   ...
                   (if (<= x1 x2)
                       ...
                       (let ((mid-y (- (min y1 y2) 160 y-track)))
                         (incf y-track 35)
                         (push (make-edge :label label :side :control :x1 x1 :y1 y1 :x2 (+ x1 40) :y2 y1) all-edges)
                         (push (make-edge :label "" :side :control :x1 (+ x1 40) :y1 y1 :x2 (+ x1 40) :y2 mid-y) all-edges)
                         (push (make-edge :label "" :side :control :x1 (+ x1 40) :y1 mid-y :x2 x2 :y2 mid-y) all-edges)
                         (push (make-edge :label "" :side :control :rev t :x1 x2 :y1 mid-y :x2 x2 :y2 y2) all-edges))))))
            (:mechanism
             (let* ((t-offset (if (> t-total 1) (- (* (/ (node-width n2) t-total) t-idx) (/ (node-width n2) 2.5)) 0))
                    (x2 (+ (node-x n2) (/ (node-width n2) 2) t-offset))
               ...
               (if (<= x1 x2)
                   ...
                   (let ((mid-y (+ (max y1 y2) 160 y-track)))
                     (incf y-track 35)
                     (push (make-edge :label label :side :mechanism :x1 x1 :y1 y1 :x2 (+ x1 40) :y2 y1) all-edges)
                     (push (make-edge :label "" :side :mechanism :x1 (+ x1 40) :y1 y1 :x2 (+ x1 40) :y2 mid-y) all-edges)
                     (push (make-edge :label "" :side :mechanism :x1 (+ x1 40) :y1 mid-y :x2 x2 :y2 mid-y) all-edges)
